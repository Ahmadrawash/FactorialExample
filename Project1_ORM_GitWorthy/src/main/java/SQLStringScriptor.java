...
public class SQLStringScriptor {
    ...
    public static String getFieldNameList(List<Column> columns) { //for Update statement
    ...
    public static String getFieldNameListForInsert(List<Column> columns) { //for insert statement (primary key is autoincrement. skip add the ID column of the UserClass
        //Create return string
        String fieldNames = "";
        ...
        if (columns.size() > 0) {
            ...
            //*************skip add the ID column of the UserClass (it is declared as autoincrement so its value will be generated automatically)
            fieldNames += columns.get(1).getFieldName();
    ...
    public static String getSetClause(List<Column> columns) {
        ...
        if (columns.size() > 0) {
            ...
            setClause += " SET " + columns.get(1).getFieldName() + " = ?";
    ...
    public static String makeCreateSQLString(Repository repository) { //*************** the INSERT INTO SQL STATEMENT
        ...
        String fieldNameList = getFieldNameListForInsert(columns);

        //Create placeholderList string to be added to SQL statement
        String placeholderList = getPlaceholderList(columns.size()-1);
    ...
    //Method to generate the SQL for an update method
    /*
    public static String makeInsertString(Repository repository) {
        //Check whether the repository table has been initialized
        //Return empty string if not
        if (!repository.isTableInitialized()) {
            return "";
